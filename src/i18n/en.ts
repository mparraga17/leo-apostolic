// ============================================================
// en.ts — English translations
// ============================================================

export default {
  // Tabs
  tabs: {
    today: 'Today',
    agenda: 'Events',
    places: 'Places',
    prayers: 'Prayers',
    songs: 'Hymns',
    shop: 'Shop',
  },

  // Today screen
  today: {
    countdownLabel: 'APOSTOLIC VISIT · MADRID',
    countdownDates: 'June 6 — 9, 2026',
    days: 'days',
    hours: 'hours',
    minutes: 'min',
    seconds: 'sec',
    saintOfTheDay: 'SAINT OF THE DAY',
    feastBadge: 'SOLEMNITY',
    popeQuote: "WORDS OF THE HOLY FATHER",
    prayerOfTheDay: 'PRAYER OF THE DAY',
    morePrayers: 'More prayers',
    blessing: 'Blessing',
    duringVisit: 'THE POPE IS IN MADRID',
    afterVisit: 'The visit has ended. May the fruit of these days remain.',
  },

  // Events screen
  agenda: {
    headerSubtitle: 'Apostolic visit · June 6-9, 2026',
    headerTitle: 'Events',
    livePapal: 'THE POPE NOW',
    publicBadge: 'PUBLIC EVENT',
    liveBadge: 'LIVE',
    publicNotice: 'Public event open to the faithful',
    registrationTitle: 'REGISTRATION REQUIRED',
    registrationButton: 'Register at madrid.conelpapa.es',
    directionsButton: 'Get directions',
    suggestionsTitle: 'SUGGESTIONS',
    timeFreeLabel: 'FREE TIME',
    timeFreeValue: '{{time}} until the next event',
    timeFreeNext: 'Next: {{title}} · {{time}}',
    nearbyTitle: 'PLACES NEARBY',
    nearbyDirections: 'Get directions',
  },

  // Places screen
  places: {
    headerSubtitle: 'Cultural and devotional Madrid',
    headerTitle: 'Places',
    featuredSection: 'MUST-SEE',
    massScheduleTitle: 'MASS SCHEDULE',
    confessionsTitle: 'CONFESSIONS',
    scheduleDisclaimer: 'Schedules may vary on holidays and during summer. Check the official website before visiting.',
    directionsButton: 'Get directions',
    websiteButton: 'Official website',
  },

  // Prayers screen
  prayers: {
    headerSubtitle: 'Timeless prayers of the Church',
    headerTitle: 'Prayers',
    showLatin: 'Latin',
    showSpanish: 'Spanish',
  },

  // Hymns screen
  songs: {
    headerSubtitle: 'Traditional and modern',
    headerTitle: 'Hymns',
    tabTraditional: 'Traditional',
    tabModern: 'Modern',
    listenSpotify: 'Listen on Spotify',
    copyrightNote: 'Modern hymns are not reproduced here out of respect for copyright. They are linked to Spotify so you can listen legally.',
  },

  // Shop screen
  shop: {
    headerSubtitle: 'Books and religious items',
    headerTitle: 'Shop',
    featuredSection: 'FEATURED',
    exploreSection: 'EXPLORE ON AMAZON',
    affiliateDisclaimer: 'As an Amazon Associate, this app may earn commissions from purchases made through the links. This does not affect the price you pay.',
  },

  // Common
  common: {
    close: 'Close',
    cancel: 'Cancel',
    ok: 'OK',
    share: 'Share',
    language: 'Language',
    languageSpanish: 'Español',
    languageEnglish: 'English',
  },
};
